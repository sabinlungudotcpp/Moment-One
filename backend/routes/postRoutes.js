...
postRouter.route('/').get(postController.getAllPosts).post(authenticate, postController.createNewPost).delete(postController.deleteAllPosts); // Post router handles a get request on the get all posts route, create a new post by sending a request to the server using a POST request
